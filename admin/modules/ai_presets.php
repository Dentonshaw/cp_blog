<?php
/**
 * AI 人设库（移植自 crush-service style_library.py）
 * 8 套预设人设：温柔粘人 / 高冷傲娇 / 热情主动 / 成熟稳重 / 甜酷拽 / 理性冷静 / 幽默风趣 / 被动慢热
 */

function ai_presets(): array {
    return [
        '温柔粘人' => [
...
function ai_resolve_preset(string $key): string {
    $presets = ai_presets();
    foreach (array_keys($presets) as $k) {
        if ($key === $k || strpos($key, $k) !== false || strpos($k, $key) !== false) {
            return $k;
        }
    }
    return '温柔粘人';
